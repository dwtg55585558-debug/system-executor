...
export default function CharacterStatusCard({
  ...
  const stateColor = risk ? "#c96660" : mission.complete ? C.sage : activated ? stage.identityAccent : "#66778a";
  const cardBorder = stage.identityAccentBorder;
  const cardBackground = activated
    ? `radial-gradient(circle at 24% 48%, ${stage.identityGlow}, transparent 38%), ${stage.activatedBackground}`
    : `radial-gradient(circle at 24% 48%, ${stage.identityGlow}, transparent 42%), ${stage.idleBackground}`;
  ...
  return (
    <Card
      style={{
        ...
        boxShadow: activated
          ? `0 16px 38px rgba(0,0,0,.4), 0 0 24px ${stage.identityGlow}, inset 0 0 0 1px ${stage.activatedInnerBorder}`
          : `0 16px 34px rgba(0,0,0,.36), 0 0 20px ${stage.identityGlow}`,
      }}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <div style={{ color: stage.identityAccent, fontFamily: FONT_MONO, fontSize: 10, letterSpacing: 1.3 }}>{stage.label} · LV.{level}</div>
      ...
      <div className="mt-3 grid items-center gap-4" style={{ gridTemplateColumns: "minmax(112px,124px) minmax(0,1fr)" }}>
        <div className="relative mx-auto" style={{ width: "clamp(112px,31vw,124px)", height: "clamp(112px,31vw,124px)" }}>
          <div style={{ width: "100%", height: "100%", borderRadius: 24, overflow: "hidden", border: `1px solid ${stage.identityAccentBorder}` }}>
            <img src={image} alt={`${stage.label}${activated ? "啟動" : "待命"}狀態`} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center" }} />
          </div>
          {mission.permission && <div className="absolute -bottom-2 left-1/2 flex -translate-x-1/2 items-center gap-1 whitespace-nowrap rounded-full px-2 py-1" style={{ color: stage.identityAccent, background: "#0a0d11", border: `1px solid ${stage.identityAccentBorder}`, fontFamily: FONT_MONO, fontSize: 8.5, fontWeight: 800 }}><ShieldCheck size={11} />TRADE {mission.tradeNumber} · AUTHORIZED</div>}
        </div>
      ...
      {mission.cta && <button type="button" onClick={onPrimary} className="mt-5 w-full rounded-xl px-4 py-3 text-sm font-bold" style={{ minHeight: 46, color: activated ? "#071018" : "#111820", background: activated ? stage.identityAccent : "#718296", boxShadow: activated ? `0 7px 16px -10px ${stage.identityAccent}` : "none" }}>{mission.cta}</button>}
